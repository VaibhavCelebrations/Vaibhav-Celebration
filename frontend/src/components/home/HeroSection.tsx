    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-10 w-full relative z-10">
        <div className="relative z-10 max-w-2xl">
          ...
          <h1 className="font-display text-4xl sm:text-5xl lg:text-[3.5rem] leading-[1.12] text-charcoal font-semibold">
            {content?.headline ? (
              content.headlineAccent && content.headline.includes(content.headlineAccent) ? (
                <>
                  {content.headline.slice(0, content.headline.indexOf(content.headlineAccent))}
                  <span className="italic font-normal text-mocha">{content.headlineAccent}</span>
                  {content.headline.slice(content.headline.indexOf(content.headlineAccent) + content.headlineAccent.length)}
                </>
              ) : (
                content.headline
              )

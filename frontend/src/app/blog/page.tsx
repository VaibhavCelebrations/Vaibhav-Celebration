            ...
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-10 mb-20">
              ...
              <div className="w-full lg:max-w-md">
                <div className="relative mb-6">
                  ...
                  <input type="text" placeholder="Search articles, tags, topics..." className="w-full pl-11 pr-4 py-3.5 bg-white border border-border-light rounded-xl text-sm focus:outline-none focus:border-mocha focus:ring-1 focus:ring-mocha transition-all shadow-sm" />
                </div>
                <div className="flex flex-wrap gap-2.5">
                  {["All topics", ...categories].map((cat, i) => (
                    <button key={cat} className={`px-4 py-2 rounded-full text-xs font-medium border transition-colors ${i === 0 ? "bg-mocha text-white border-mocha" : "bg-white text-text-muted border-border-light hover:border-mocha/50 hover:text-charcoal"}`}>
                      {cat}
                    </button>
                  ))}
                </div>
              ...
              <div className="mb-20">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-sm text-text-muted">
                    {featuredPost.isFeatured ? "Editor's pick" : "Newest insight"}
                  </span>
                </div>
                <Link href={`/blog/${featuredPost.slug}`} className="group flex flex-col md:flex-row bg-white rounded-2xl overflow-hidden border border-border-light shadow-sm hover:shadow-hover transition-all duration-300">
                  <div className="w-full md:w-[55%] relative aspect-[4/3] md:aspect-auto">
                    <Image src={featuredPost.coverImage} alt={featuredPost.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 55vw" priority />
                  </div>
                  <div className="w-full md:w-[45%] p-8 lg:p-12 flex flex-col justify-center">
                    ...
                    <h3 className="font-display text-3xl lg:text-4xl font-bold text-charcoal leading-[1.2] mb-4 group-hover:text-mocha transition-colors">{featuredPost.title}</h3>
                    <p className="text-text-muted text-base leading-relaxed mb-8 line-clamp-3">{featuredPost.shortDescription}</p>
                ...
                {regularPosts.map((post) => (
                  <Link href={`/blog/${post.slug}`} key={post.id} className="group flex flex-col bg-white rounded-2xl overflow-hidden border border-border-light shadow-sm hover:shadow-hover transition-all duration-300 hover:-translate-y-1 h-full">
                    <div className="relative aspect-[16/9] w-full overflow-hidden">
                      <Image src={post.coverImage} alt={post.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                    </div>
                    <div className="p-6 md:p-8 flex flex-col flex-grow">
                      ...
                      <h3 className="font-display text-xl font-bold text-charcoal leading-snug mb-3 group-hover:text-mocha transition-colors">{post.title}</h3>
                      <p className="text-text-muted text-sm leading-relaxed line-clamp-2 mb-8">{post.shortDescription}</p>
                      <div className="mt-auto pt-4 border-t border-border-light/50 flex items-center justify-between text-xs font-medium text-text-muted">
                        ...
                        <span className="inline-flex items-center gap-1 group-hover:text-mocha transition-colors text-charcoal font-semibold">Read <ArrowRight size={14} /></span>
                      </div>
                    </div>
                  </Link>
                ))}

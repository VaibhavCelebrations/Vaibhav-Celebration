...
export default async function EventsPage() {
  ...
  const regularEvents = featuredEvent ? events.filter((e) => e.id !== featuredEvent.id) : events;
    ...
    <>
      ...
      <main className="pt-28 md:pt-36 pb-20 bg-cream min-h-screen">
        <div className="max-w-6xl mx-auto px-5 md:px-10">
          ...
          {/* Featured Event - Horizontal Display (similar like blog) */}
          {featuredEvent && (
              ...
              <div className="mb-16 md:mb-20">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2.5">
                    ...
                    <h2 className="font-display text-2xl md:text-3xl text-charcoal font-bold">Featured Event</h2>
                  </div>
                  <span className="text-xs font-semibold text-mocha uppercase tracking-wider bg-white border border-mocha/20 px-3.5 py-1 rounded-full shadow-sm">
                    Signature Highlight
                  </span>
                </div>
          ...
          {regularEvents.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-8">
                <h2 className="font-display text-2xl md:text-3xl text-charcoal font-bold">
                  All Celebrations
                </h2>
                <span className="text-sm text-text-muted">
                  {regularEvents.length} {regularEvents.length === 1 ? "celebration" : "celebrations"}
                </span>
              </div>

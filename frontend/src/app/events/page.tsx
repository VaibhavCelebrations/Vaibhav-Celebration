          ...
          <ScrollReveal>
            <div className="text-center mb-16">
              <div className="flex items-center justify-center gap-4 mb-4">
                ...
                <p className="text-sm font-bold text-mocha uppercase tracking-[0.2em]">Portfolio</p>
                <div className="h-px w-10 md:w-16 bg-gradient-to-l from-transparent to-mocha/60" />
              </div>
              <h1 className="font-display text-4xl md:text-5xl lg:text-6xl text-charcoal font-semibold mb-6">Our Magical Events</h1>
            ...
            {events.map((event, i) => (
              <ScrollReveal key={event.id} delay={i * 80}>
                <Link href={`/events/${event.slug}`} className="group block relative rounded-[2rem] overflow-hidden shadow-card aspect-[4/3] hover:shadow-hover transition-all duration-300 hover:-translate-y-2">
                  <Image src={event.coverImage} alt={event.title} fill className="object-cover transition-transform duration-1000 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" priority={i < 3} />
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-end text-left">
                    ...
                    <h3 className="font-display text-2xl lg:text-3xl font-bold !text-white mb-2 group-hover:text-gold-soft transition-colors leading-[1.15]">{event.title}</h3>
                    <p className="text-white/80 text-sm leading-relaxed line-clamp-2 mb-6">{event.shortDescription}</p>
